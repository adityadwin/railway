...
const registerUser = async (req, res, next) => {
  try {
    ...
    if (user) {
      throw new Error("User sudah registrasi");
    }
...
const userProfile = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      let error = new Error("User tidak ditemukan");
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    if (req.body.password && req.body.password.length < 6) {
      throw new Error("Panjang password minimal haris 6 karakter");
    ...
    upload(req, res, async function (err) {
      if (err) {
        const error = new Error(
          "Terjadi kesalahan ketika mengunggah foto " + err.message
        );
        next(error);
      } else {
...
const deleteUser = async (req, res, next) => {
  try {
    ...
    res.status(204).json({ message: "User berhasil dihapus" });
  } catch (error) {
    next(error);
  }
};
